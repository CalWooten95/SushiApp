...
def addItem(id):
    db = get_db()
    user_id = session.get('user_id')
    uname = db.execute('SELECT username FROM user WHERE id = ?', (user_id,)).fetchone()
    itemn = db.execute('SELECT itemName FROM items WHERE iid = ?', (id,)).fetchone()
    u = uname['username']
    t = itemn['itemName']
    t1 = time.time()
    db.execute('INSERT INTO orderedItems (iid, uid, nameofitem, username, mark, active) VALUES (?, ?, ?, ? ,?, 1)', (id, user_id, t, u, t1))
    db.commit()
